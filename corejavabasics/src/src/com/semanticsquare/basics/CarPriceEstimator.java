...
public class CarPriceEstimator {
	...
	public double getSalePrice(String makeAndModel, int yearManufactured, double milesDriven, 
		...
		// then reduce price by $1000. So, if all features are there,
		// then we do not reduce price, i.e., we skip the body. 
		// In other words, even if one the security features is not
		// there, then we reduce the price.  
		...
		//        reduce sale price by 1500
                ...
                if ( additionalMiles > 1000.0 && additionalMiles <= 10000.0) {
	...
	public static void main(String[] args) {
		...
		double salesPrice = carPriceEstimator.getSalePrice("ford ecosport", 2024, 5000.0, 2, true, true, true, false, false, false);		
	}
}
